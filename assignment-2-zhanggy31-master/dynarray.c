...
struct dynarray {
  ...
  int size;
  int capacity;
  int front;/*present the front of queue*/
};
...
struct dynarray* dynarray_create() {
  ...
  da->front = 0; /*set front at the front of queue, pos[0]*/
  return da;
}
...
/*check queue if it's emptyu*/
int dynarray_isempty(struct dynarray* da){
  return da->size==0;
}

/*enqueue4queue is used to enqueue*/
void dynarray_enqueue4queue(struct dynarray* da, void* val){
  if(da->size == da->capacity){
    int cap = 2 * da->capacity;
    void** temp_data = malloc(cap * sizeof(void*));
    for(int i=0;i<da->capacity;i++){
      int physical_pos = (da->front+i) % da->capacity;
      temp_data[i]=da->data[physical_pos];
    }
    free(da->data);
    da->data = temp_data;
    da->front = 0;
  ...
  int addr =(da->front+ da->size)%da->capacity;
  da->data[addr]= val;
  da->size = da->size + 1;  
}

/*queue_front is used to get the front data of queue*/
void* dynarray_queue_front(struct dynarray* da){
    return da->data[da->front];
}

/*dequeue4queue is used to dequeue*/
void* dynarray_dequeue4queue(struct dynarray* da){
  void* value = da->data[da->front];
